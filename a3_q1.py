...
while True:
  ...
  print(
    "5. Calculate the total number of ballots cast in every Canadian province and territory"
  )
  print("6. Sort the dataset based on a specific attribute")
  print(
    "7. Binary search to find the percentage of voter turnout for an electoral district"
  )
  print("0. Exit\n")
  option = input("Option: ")
  ...
  if option == "0":
    ...
    key = input(
      "Enter attribute name (Electors/Population/Polling Stations/Valid Ballots/Rejected Ballots/Total Ballots Cast): "
    )
    maxVal, maxED = functions3.findMax(data, key)
    print("Maximum value of", key, "is", maxVal, "in electoral district",
          maxED)
    ...
    print("Minimum value of", key, "is", minVal, "in electoral district",
          minED)

  elif option == "5":
    provinces = functions3.totalVotes(data)
    print(
      "Total number of ballots cast in every Canadian province and territory:")
    ...
    if percTurnout == -1:
      ...
      print("Percentage of voter turnout for electoral district", edNum, "is",
            percTurnout)
